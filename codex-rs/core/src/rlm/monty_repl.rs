...
const HOST_CALLBACK_FUNCTIONS: [&str; 6] = [
    "llm_query",
    "llm_query_batched",
    "read_file",
    "write_file",
...
impl MontyHostCallbacks for StubHostCallbacks {
    ...
    fn llm_query_batched(
        ...
        Err(MontyReplError::UnsupportedHostCallback(
            "llm_query_batched callback is not wired yet".to_string(),
        ))
    }
...
impl MontyReplRuntime {
    ...
    fn dispatch_function_call(
        ...
        if !kwargs.is_empty() && !matches!(function_name, "llm_query" | "llm_query_batched") {
            return Err(MontyReplError::InvalidHostCall(format!(
        ...
        match function_name {
            "llm_query" => {
                let (prompt, schema) = Self::llm_query_args(&args, &kwargs)?;
                callbacks.llm_query(&prompt, &schema)
            }
            "llm_query_batched" => {
                let (prompts, schema) = Self::llm_query_batched_args(&args, &kwargs)?;
                callbacks.llm_query_batched(&prompts, &schema)
            }
    ...
    fn llm_query_args(
    ...
    ) -> Result<(String, MontyObject), MontyReplError> {
        if args.is_empty() {
            return Err(MontyReplError::InvalidHostCall(
                "llm_query expected a prompt argument at index 0".to_string(),
            ));
        }
        if args.len() > 2 {
            return Err(MontyReplError::InvalidHostCall(
                "llm_query accepts at most 2 positional arguments: prompt and schema".to_string(),
            ));
        }
        let prompt = match &args[0] {
            ...
            _ => {
                return Err(MontyReplError::InvalidHostCall(
                    "llm_query expected a string prompt argument at index 0".to_string(),
                ));
            }
        };
        let positional_schema = args.get(1).cloned();
        let mut keyword_schema = None;
        for (key, value) in kwargs {
            let MontyObject::String(keyword) = key else {
                return Err(MontyReplError::InvalidHostCall(
                    "llm_query keyword names must be strings".to_string(),
                ));
            };
            if keyword != "schema" {
                return Err(MontyReplError::InvalidHostCall(format!(
                    "llm_query received unsupported keyword argument `{keyword}`"
                )));
            }
            if keyword_schema.is_some() {
                return Err(MontyReplError::InvalidHostCall(
                    "llm_query received duplicate `schema` keyword argument".to_string(),
                ));
            }
            keyword_schema = Some(value.clone());
        }
        if positional_schema.is_some() && keyword_schema.is_some() {
            return Err(MontyReplError::InvalidHostCall(
                "llm_query accepts schema either positionally or as keyword, not both".to_string(),
            ));
        }
        let schema = positional_schema.or(keyword_schema).ok_or_else(|| {
            MontyReplError::InvalidHostCall(
                "llm_query requires a schema argument (use llm_query(prompt, schema={...}))"
                    .to_string(),
            )
        })?;
        Ok((prompt, schema))
    ...
    fn llm_query_batched_args(
        args: &[MontyObject],
        kwargs: &[(MontyObject, MontyObject)],
    ) -> Result<(Vec<String>, MontyObject), MontyReplError> {
        if args.is_empty() {
            return Err(MontyReplError::InvalidHostCall(
                "llm_query_batched expected a prompts argument at index 0".to_string(),
            ));
        }
        if args.len() > 2 {
            return Err(MontyReplError::InvalidHostCall(
                "llm_query_batched accepts at most 2 positional arguments: prompts and schema"
                    .to_string(),
            ));
        }
        let prompts = match &args[0] {
            MontyObject::List(items) | MontyObject::Tuple(items) => {
                ...
                for item in items {
                    let MontyObject::String(prompt) = item else {
                        return Err(MontyReplError::InvalidHostCall(
                            "llm_query_batched prompts must be a list of strings".to_string(),
                        ));
                    };
            ...
            _ => {
                return Err(MontyReplError::InvalidHostCall(
                    "llm_query_batched expected prompts to be a list of strings".to_string(),
                ));
            }
        };
        if prompts.is_empty() {
        ...
        for (key, value) in kwargs {
            let MontyObject::String(keyword) = key else {
                return Err(MontyReplError::InvalidHostCall(
                    "llm_query_batched keyword names must be strings".to_string(),
                ));
            };
            if keyword != "schema" {
                return Err(MontyReplError::InvalidHostCall(format!(
                    "llm_query_batched received unsupported keyword argument `{keyword}`"
                )));
            }
            if keyword_schema.is_some() {
                return Err(MontyReplError::InvalidHostCall(
                    "llm_query_batched received duplicate `schema` keyword argument".to_string(),
                ));
            }
            keyword_schema = Some(value.clone());
        }
        if positional_schema.is_some() && keyword_schema.is_some() {
            return Err(MontyReplError::InvalidHostCall(
                "llm_query_batched accepts schema either positionally or as keyword, not both"
                    .to_string(),
            ));
        }
        let schema = positional_schema.or(keyword_schema).ok_or_else(|| {
            MontyReplError::InvalidHostCall(
                "llm_query_batched requires a schema argument (use llm_query_batched(prompts, schema={...}))"
                    .to_string(),
            )
        })?;
        Ok((prompts, schema))
...
mod tests {
    ...
    impl MontyHostCallbacks for TestHostCallbacks {
        ...
        fn llm_query_batched(
            ...
            Ok(MontyObject::List(
                prompts
                    .iter()
                    .map(|prompt| MontyObject::String(format!("llm:{prompt}")))
                    .collect(),
            ))
        }
    ...
    fn execute_code_routes_host_callback_calls() -> Result<(), MontyReplError> {
        ...
        let result = repl.execute_code_with_callbacks(
            "llm_query('prompt text', schema={'kind': 'string'})",
            &TestHostCallbacks,
        )?;

        assert_eq!(
            result.value,
            MontyObject::String("llm:prompt text".to_string())
        );
        Ok(())
    }
    ...
    #[test]
    fn llm_query_requires_schema_argument() -> Result<(), MontyReplError> {
        let mut repl = MontyReplRuntime::new()?;

        let error = repl
            .execute_code_with_callbacks("llm_query('prompt')", &TestHostCallbacks)
            .expect_err("schema should be required");
        assert!(error.to_string().contains("requires a schema argument"));
        Ok(())
    }

    #[test]
    fn llm_query_batched_requires_schema_argument() -> Result<(), MontyReplError> {
        let mut repl = MontyReplRuntime::new()?;

        let error = repl
            .execute_code_with_callbacks("llm_query_batched(['a', 'b'])", &TestHostCallbacks)
            .expect_err("schema should be required");
        assert!(error.to_string().contains("requires a schema argument"));
        Ok(())
    }
    ...
    #[test]
    fn llm_query_batched_routes_host_callback_calls() -> Result<(), MontyReplError> {
        let mut repl = MontyReplRuntime::new()?;

        let result = repl.execute_code_with_callbacks(
            "llm_query_batched(['a', 'b'], schema={'kind': 'string'})",
            &TestHostCallbacks,
        )?;
        ...
        assert_eq!(
            ...
            MontyObject::List(vec![
                MontyObject::String("llm:a".to_string()),
                MontyObject::String("llm:b".to_string()),
            ])
        );
        Ok(())
    }
