...
const SCRIPT_NAME: &str = "<codex-python-repl>";
const HOST_CALLBACK_FUNCTIONS: [&str; 4] = ["read_file", "write_file", "list_files", "shell"];
...
impl SessionMutationGate {
    pub(crate) fn with_read<T>(
        ...
        debug!(
            ...
            "python_repl read callback gate"
        );
        drop(guard);
    ...
    pub(crate) fn with_write<T>(
        ...
        debug!(
            wait_ms = waited.as_millis(),
            hold_ms = held.as_millis(),
            "python_repl write callback gate"
        );
        drop(guard);
